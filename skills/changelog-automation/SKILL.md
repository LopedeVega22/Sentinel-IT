...
## ACTIVE DOCUMENTATION TRACKING (CONTINUOUS UPDATES)

To ensure complete and persistent documentation, you must act proactively as an AI agent:
- **Whenever you make a change to the codebase** (new features, modifications, fixes), you MUST remember to document what is new or distinct.
- Proactively ask the user if they want you to update the `CHANGELOG.md` file using the **Keep a Changelog** format described above.
- Always categorize the changes properly (Added, Changed, Deprecated, Removed, Fixed, Security).
- Never consider a feature complete until the `CHANGELOG.md` has been updated to reflect its current state.
