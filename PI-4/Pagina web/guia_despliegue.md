# Guía de Despliegue - CyberGuard

Esta guía explica los pasos necesarios para instalar y poner en marcha el panel web de CyberGuard y preparar la base de datos.
...
2. Crea una nueva base de datos llamada `cyberguard` (o el nombre que prefieras).
3. Importa el archivo `schema.sql` proporcionado en el código fuente.
   - *Por consola:* `mysql -u root -p cyberguard < schema.sql`
...
3. Verifica y ajusta las credenciales de conexión según tu entorno local:
   ```php
   $host = 'localhost';
   $db   = 'cyberguard'; // El nombre de tu base de datos
   $user = 'root';       // Tu usuario de MySQL/MariaDB
   $pass = '';           // Tu contraseña de MySQL/MariaDB
   ```

## 4. Acceso al Panel
1. Abre tu navegador y dirígete a `http://localhost/cyberguard-web` (o la ruta donde hayas colocado el proyecto).
...
3. Las credenciales de prueba por defecto (si no has modificado los hashes en la base de datos) son:
   - **Administrador:** admin@cyberguard.com / admin123
   - **Clientes:** cliente1@cyberguard.com o cliente2@cyberguard.com / cliente123
